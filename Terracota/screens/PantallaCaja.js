...
export default function PantallaCaja({
  ...
  const compartirTicket = async () => {
    if (!ticketSeleccionado || compartiendo) return;

    try {
      setCompartiendo(true);
      await compartirTicketPdf(ticketSeleccionado);
    } catch (error) {
      console.error('No fue posible generar el PDF del ticket:', error);
      Alert.alert(
    ...
    <MarcoTelefono elementosNavegacion={navegacionPorRol.caja} activo={pantalla} alNavegar={cambiarPantalla}>
      ...
      {pantalla === 'ticketDetalle' && ticketSeleccionado && (
        <Contenido>
          ...
          <TouchableOpacity
            style={[styles.shareButton, compartiendo && styles.shareButtonDisabled]}
            onPress={compartirTicket}
            activeOpacity={0.82}
            disabled={compartiendo}
            accessibilityRole="button"
            accessibilityLabel="Generar y compartir ticket en PDF">
            {compartiendo
              ? <ActivityIndicator size="small" color={colores.surface} />
              : <Icono icono="compartir" tono="light" tamaño={16} />}
            <Text style={styles.shareText}>
              {compartiendo ? 'Generando PDF...' : Platform.OS === 'web' ? 'Guardar como PDF' : 'Compartir PDF'}
            </Text>
          </TouchableOpacity>
        </Contenido>
      )}
