...
interactiva queda disponible en `http://localhost:8080/docs`.

## Inicio rápido con Docker

Desde la carpeta `Terracota`:

```bash
docker compose up --build
```

Esto inicia PostgreSQL en el puerto `5432` y el API en `8080`. El volumen de
...
el flujo Mesero -> Cocina -> Mesero -> Caja. Ejecuta las solicitudes en el orden
en que aparecen; la colección guarda automáticamente el identificador del pedido.
